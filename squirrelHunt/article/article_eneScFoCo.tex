...
\includegraphics[width=0.6\linewidth]{Screenshot_Example.png}

En aquest mateix directori també tindrem tots els fitxers generats per l'sdcc.

\section{Configuració}

Hi ha dos fitxers principals de configuració, el \textbf{project\_config.js} que conté aspectes relatius a la forma de compilar el projecte, el que seria el \textit{makefile} en altres projectes:
\begin{itemize}
\item Generació del binari final en diferents formats (ROM, BIN, COM…).
\item Configuració de la màquina objectiu (MSX1, MSX2, 2+, Turbo-R…).
\item Opcions d'optimització i depuració per ajustar velocitat, mida o mode debug.
\item Gestió d'arxius addicionals (fitxers binaris, imatges de disc).
\item Execució automàtica en emulador amb possibilitat d'afegir extensions (SCC, MSX-Music, OPL4, V9990…).
\end{itemize}

El \textbf{MSXgl} té un disseny \textbf{modular}, pensat perquè cada projecte pugui activar només
les parts de la llibreria que necessita. Això permet obtenir programes més lleugers, tant en temps d’execució com en consum de memòria.

Tots aquests mòduls es configuren al fitxer \textbf{msxgl\_config.h}, on podem definir:
\begin{itemize}
\item \textbf{Objectiu i màquina}: Selecció del format de programa (ROM, BIN, COM, MegaROM…) i la versió de MSX (MSX1, MSX2, Turbo R, etc.).
\item \textbf{BIOS i VDP}: Definir com accedir a les rutines de BIOS (directe o interslot).Configurar el xip de vídeo (VDP): modes de pantalla permesos, ús de sprites, comandes, paletes, protecció ISR…
\item \textbf{Mòduls addicionals}:
  \begin{itemize}
    \item \textbf{Input}: teclat, joystick, ratolí, paddle, proteccions ISR.
    \item \textbf{Memòria}: funcions ràpides de memcpy/set, gestió dinàmica, validacions.
    \item \textbf{MSX-DOS}: suport per fitxers amb FCB, handles i utilitats.
    \item \textbf{Rellotge} RTC: data/hora, dades en CMOS, validació.
    \item \textbf{Print i Text}: fonts de text, bitmap, VRAM, sprites, efectes (ombra, outline), printf.
    \item \textbf{Sprites FX}: flip, crop, màscara, rotació.
    \item \textbf{Game framework}: estat del joc, bucle principal, vsync.
    \item \textbf{GamePawn}: físics, col·lisions, tilemap, límits de pantalla.
    \item \textbf{Menus}: definició d’interfícies de menú amb callbacks.
    \item \textbf{Scroll i Tilemap}: scrolling horitzontal/vertical, ajust per píxel, màscares amb sprites.
    \item \textbf{Audio}: PSG, SCC, MSX-Music, MSX-Audio, replayers (PT3, TriloTracker, WYZ, VGM, LVGM, PCM…).
    \item \textbf{Compressió}: suport per RLEp, Pletter, Bitbuster, ZX0, LZ48, MSXi.
    \item \textbf{Altres}: NinjaTap, PAC (save SRAM), generació de QR Codes, mòdul de cadenes i sprintf, matemàtiques i random.
  \end{itemize}
\item \textbf{Debug i Perfilat}: Opcions per activar debugging amb Emulicious o openMSX i perfilar rendiment.
\end{itemize}
...
\begin{tcolorbox}[
  enhanced,
  sidebyside, sidebyside gap=8mm,
  listing side text,      % code on the left, text on the right
...
\end{lstlisting}
\tcblower
These four lines configure the video display processor (VDP)
to use 8-bit units and set the screen resolution to 256×256
before calling the initialization routine.
\end{tcolorbox}
...
These four lines configure the video display processor
to use 8-bit units and set the screen resolution to 256×256
before calling the initialization routine.
\end{minipage}

\begin{tcolorbox}[listing only,listing options={language=C}]
\#define VDP\_UNIT    
\#define SCREEN\_W    256
\#define SCREEN\_H    256
init\_vdp();
\end{tcolorbox}

\smallskip
\textit{Explanation:} These four lines configure the VDP...
